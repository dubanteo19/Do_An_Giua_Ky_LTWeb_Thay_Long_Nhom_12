...
public class VerificationDAO extends AbtractDAO<Verification> implements IVerificationDAO {
	...
	public int save(Verification verification) {
		String sql = "INSERT INTO verifications(user_id,token,exp_date) VALUES (?,?,?)";
	...
	public List<Verification> findById(int id) {
		String sql = "SELECT * FROM verifications WHERE user_id =?";
	...
	public static void main(String[] args) {
		...
		Verification verification = new Verification(1, "abc", new Date(javaDate.getTime()));
		verificationDAO.save(verification);
	}

	@Override
	public boolean verify(int userId, String token) {
		String sql = "SELECT * FROM verifications WHERE user_id =? and token =?";
